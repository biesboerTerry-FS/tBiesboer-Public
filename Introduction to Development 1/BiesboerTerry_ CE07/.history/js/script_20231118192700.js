...
document.querySelector("#calcBMI").addEventListener("click", function(e){
    ...
    )
    {
        ...
        let heightTotal = parseFloat((heightFt*12) + heightIn);

        let bodyMassIndex = parseFloat((( weightLbs / heightTotal ) / heightTotal )*703);
    
        if(bodyMassIndex >= 0 && bodyMassIndex <=18.49)
        {
            document.querySelector("#finalResult").innerHTML = `Your BMI is ${bmi.toFixed(1)} , indicating your weight is Under Weight Range`;
             document.querySelector("#image").src ="img/underWeight.jpg";
        }
        }

//alert("File is connected");

/* Terry Biesboer
November 13, 2023
CE06 HTML Forms */



let numGals = parseInt(document.querySelector("#numGals"))

let quartsAmount = numGals/4;
let pintsAmount = numGals/8;
let cupsAmount = numGals/16;



if(document.querySelector("#inQuarts").checked)
{
    console.log("Quarts is checked");
}
if(document,querySelector("#inPints").checked)
{
    console.log("Pints is checked");
}
if(document.querySelector("#inCups").checked)
{
    console.log("Cups is cxhecked");
};

...
let numGals = parseFloat(document.querySelector("#numGals").value .reportValidity());
...
document.querySelector("#startCalc").addEventListener("click", function(e)
{
    if(document.querySelector("#inQuarts").checked);
     let quartsAmount = parseFloat(document.querySelector("#numGals")).value = numGals/4;
    }
    {
    else if(document.querySelector("#inPints").checked);
    let pintsAmount = parseFloat(document.querySelector("#numGals").value = numGals/8);
    }
{
      else if(document.querySelector("inCups").checked);
    let cupsAmount = parseFloat(document.querySelector("#numGals").value = numGals/16);
});
//    {
